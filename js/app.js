...
const FamilyApp = {
    ...
    async bootstrapData() {
        const [calendarData, pantryData] = await Promise.all([
            this.api.loadCalendar(),
            this.api.loadPantry()
        ]);

        this.state.menu = calendarData?.calendar || {};
        this.state.shopping = pantryData?.pantry || [];
        ensureConfigIntegrity(this.state);
    },
...
FamilyApp.api = createApiService(FamilyApp);
FamilyApp.auth = createAuth(FamilyApp);
FamilyApp.ai = createAi(FamilyApp);
FamilyApp.actions = createActions(FamilyApp);
...
document.addEventListener('keydown', (event) => {
    ...
    if (target.id === 'accessCodeInput' || target.id === 'usernameInput') {
        FamilyApp.auth.requestToken();
    }
});
